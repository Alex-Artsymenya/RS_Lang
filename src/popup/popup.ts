...
export class PopUp {
        drawPopUpd(create: boolean = true): void {
            const popupHTML = `
                <div class="popup_inner">
                    ...
                    <div class="popup-material" id="popup-material">
                        ...
                        <label for="check-login">
                            <input type="checkbox" id="check-login" name="check-login" ${create? 'checked': ''}>
    ...
    addListeners() {
        ...
        (<HTMLElement>document.querySelector('#check-login')).addEventListener('change', () => this.turnLoginCreate());
    ...
    async run(create: boolean = true) {
        this.drawPopUpd(create);
        if (!(<HTMLElement>document.querySelector('#popup_background')).classList.contains('active')) {
