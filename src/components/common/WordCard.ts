...
class WordCard implements Component {
  ...
  public async render(): Promise<string> {
    ...
    const view = `
    <div class="word-card__card card-difficulty-${
      this.diff
    } group${groupX}" id="${this.id}">
      <div class="word-card__upper" style="background-image: url('${
        this.image
      }');">
      ...
      <div class="word-card_lower">
        ...
        <hr>
        <div class="card-text meaning_russian">${
          this.textMeaningTranslate
        }</div>
        <div class="card-text example_russian">${
      ...
      <div class="${
        groupX === 6
          ...
          : "difficulty-buttons-inactive"
      }">
      <div class="diff-button diff-hard" style="${
        this.diff === 0 ? "pointer-events:none" : "pointer-events:''"
      }">${this.diff === 2 ? "Убрать из сложного" : "Добавить в сложное"}</div>
      <div class="diff-button diff-learned" style="${
        this.diff === 2 ? "pointer-events:none" : "pointer-events:''"
      }">${
      this.diff === 0 ? "Убрать из изученного" : "Добавить в изученное"
    }</div> 
    </div>
    </div>`;
    return view;
  ...
  private async changeWordDifficultyStatus(this: HTMLElement): Promise<void> {
    ...
    const total_diff = Number(
      localStorage.getItem("rslang-current-page-total-difficulty")
    );
    if (card.classList.contains(`${CardDifficultyStyle.NORMAL}`)) {
      ...
      (parent.children[1] as HTMLElement).setAttribute(
        "style",
        "pointer-events:none"
      );
      ...
      if (total_diff - 1 !== 0) {
        (
          document.querySelectorAll(".game__button") as NodeListOf<HTMLElement>
        ).forEach((el) => el.setAttribute("style", 'pointer-events:""'));
      }
    } else {
  ...
  private async changeWordLearnedStatus(this: HTMLElement): Promise<void> {
    ...
    if (card.classList.contains(`${CardDifficultyStyle.NORMAL}`)) {
      ...
      (parent.children[0] as HTMLElement).setAttribute(
        "style",
        "pointer-events:none"
      );
      await Request.editWordInUserWordsList(
  ...
  public async after_render(): Promise<void> {
    document
      .querySelectorAll(".diff-hard")
      .forEach((el) =>
        el.addEventListener("click", this.changeWordDifficultyStatus)
      );
    document
      .querySelectorAll(".diff-learned")
      .forEach((el) =>
        el.addEventListener("click", this.changeWordLearnedStatus)
      );
    return;
  }
}
